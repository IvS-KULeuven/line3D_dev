subroutine benchmark13_solution
!
!...------calculating 3d line radiative transfer for spherically--------
...
use dime3d, only: t3d, eps_cont3d, mintbar3d, int3d, opac3d, opalbar3d, scont3d, sline3d, ssobo3d, &
                  velx3d, vely3d, velz3d, vth3d, ndxmax, ndymax, ndzmax, x, y, z, &
                  imask_innreg3d, aloline_nn3d, normalization3d, imask3d, imask_totreg3d
...
use options, only: opt_ng_line, opt_ait_line, input_mod_dim, opt_opal
...
use mod_interp2d, only: interpolation_threshold, wpa_interp2d, wpb_interp2d, wp_interp2d, &
                        wpa_interp1d, wpb_interp1d, wp_interp1d, wpa_integ1d, wpb_integ1d, wp_integ1d, lng_expol
use options, only: opt_start_line
!
implicit none
!
! ... local scalars
integer(i4b) :: i, j, k, l, muindx, err, it_start
...
if(.not.check_fname) stop 'error in benchmark13: 1d solution file does not exist'
!
...
if(abs(teff-teff_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'mdot', xmloss, xmloss_jo
if(abs(xmloss-xmloss_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'vmin', vmin, vmin_jo
if(abs(vmin*1.d5-vmin_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'vmax', vmax, vmax_jo
if(abs(vmax*1.d5-vmax_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'beta', beta, beta_jo
if(abs(beta-beta_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'yhe', yhe, yhe_jo
if(abs(yhe-yhe_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'hei', hei, hei_jo
if(abs(hei-hei_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'rstar', sr, sr_jo
if(abs(sr-sr_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'eps_cont', eps_cont, eps_cont_jo
if(abs(eps_cont-eps_cont_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'kcont', kcont, kcont_jo
if(abs(kcont-kcont_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
xnue0_jo=cgs_clight/lambda_jo/1.d-8
write(*,'(a20, 2es20.8)') 'xnue', xnue0, xnue0_jo
if(abs(xnue0-xnue0_jo)/xnue0.gt.1.d-6) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2es20.8)') 'xic1', xic1, xic1_jo
if(abs(xic1-xic1_jo).gt.1.d-5) then
   write(*,*) 'error in benchmark13: input data not consistent'
...
if(abs(eps_line-eps_line_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
write(*,'(a20, 2i20)') 'opt_opal', opt_opal, opt_opal_jo
if(opt_opal.ne.opt_opal_jo) stop 'error in benchmark13: input data not consistent'
if(opt_opal.eq.0) then
   write(*,'(a20, 2es20.8)') 'kline', kline, kline_jo
   if(abs(kline-kline_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
elseif(opt_opal.eq.1) then
   write(*,'(a20, 2es20.8)') 'kappa0', kappa0, kappa0_jo
   if(abs(kappa0-kappa0_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
   write(*,'(a20, 2es20.8)') 'alpha', alpha, alpha_jo
   if(abs(alpha-alpha_jo).gt.1.d-10) stop 'error in benchmark13: input data not consistent'
endif
...
allocate(sline1d_jo(n1d_cr), stat=err)
   if(err.ne.0) stop 'allocation error in benchmark13'
...
!open(1, file='nohup1.out', form='formatted')
!   do i=1, 62
!      read(1,*) epsmaxl_sc(i)
!   enddo
!close(1)
...
!-----------------------------------------------------------------------
!
   write(*,fmt='(a5, 7(a20))') '#', 'z', 'jbar(sc)', 'sline(sc)', 'sline(sobo)', 'deviation(old-new)', 'normalization', 'alo_diag'
...
!-----------------------------------------------------------------------
!
!store everything in benchmark13-arrays
...
sline3d_fvm = sline3d
!
end subroutine benchmark13_solution
