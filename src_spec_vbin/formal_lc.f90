...
use mod_spectrum, only: nz_ray, z_ray, opalbar_ray, opac_ray, sline_ray, scont_ray, &
                  velz_ray, profile_ray, vth_ray, temp_ray, imask_ray
!
implicit none
!
! ... arguments
...
if(allocated(imask_ray)) deallocate(imask_ray)
!
!-----------------------------------------------------------------------
!
...
allocate(imask_ray(nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: imask_ray'
...
!-----------------------------------------------------------------------
!
subroutine allocate_fs1dcs1
!
!----------------set up one arbitrary ray (for test purposes)-----------
!
use prog_type
use mod_spectrum, only: cs1_nz_ray, cs1_z_ray, cs1_opalbar_ray, cs1_opac_ray, cs1_sline_ray, cs1_scont_ray, &
                  cs1_velz_ray, cs1_vth_ray, cs1_temp_ray, cs1_imask_ray
!
implicit none
!
! ... arguments
...
!-----------------------------------------------------------------------
!
if(allocated(cs1_z_ray)) deallocate(cs1_z_ray)
if(allocated(cs1_opalbar_ray)) deallocate(cs1_opalbar_ray)
if(allocated(cs1_opac_ray)) deallocate(cs1_opac_ray)
if(allocated(cs1_sline_ray)) deallocate(cs1_sline_ray)
if(allocated(cs1_scont_ray)) deallocate(cs1_scont_ray)
if(allocated(cs1_velz_ray)) deallocate(cs1_velz_ray)
if(allocated(cs1_vth_ray)) deallocate(cs1_vth_ray)
if(allocated(cs1_temp_ray)) deallocate(cs1_temp_ray)
if(allocated(cs1_imask_ray)) deallocate(cs1_imask_ray)
!
!-----------------------------------------------------------------------
!
allocate(cs1_z_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_z_ray'
allocate(cs1_opalbar_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_opalbar_ray'
allocate(cs1_opac_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_opac_ray'
allocate(cs1_sline_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_sline_ray'
allocate(cs1_scont_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_scont_ray'
allocate(cs1_velz_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_velz_ray'
allocate(cs1_vth_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_vth_ray'
allocate(cs1_temp_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_temp_ray'
allocate(cs1_imask_ray(cs1_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs1_imask_ray'


end subroutine allocate_fs1dcs1
...
!-----------------------------------------------------------------------
!
subroutine deallocate_fs1dcs1
!
!----------------set up one arbitrary ray (for test purposes)-----------
!
use prog_type
use mod_spectrum, only: cs1_nz_ray, cs1_z_ray, cs1_opalbar_ray, cs1_opac_ray, cs1_sline_ray, cs1_scont_ray, &
                  cs1_velz_ray, cs1_imask_ray, cs1_vth_ray, cs1_temp_ray
!
implicit none
!
! ... arguments
...
!-----------------------------------------------------------------------
!
deallocate(cs1_z_ray)
deallocate(cs1_opalbar_ray)
deallocate(cs1_opac_ray)
deallocate(cs1_sline_ray)
deallocate(cs1_scont_ray)
deallocate(cs1_velz_ray)
deallocate(cs1_vth_ray)
deallocate(cs1_temp_ray)
deallocate(cs1_imask_ray)
!
!
!
end subroutine deallocate_fs1dcs1
!
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
!
subroutine allocate_fs1dcs2
!
!----------------set up one arbitrary ray (for test purposes)-----------
!
use prog_type
use mod_spectrum, only: cs2_nz_ray, cs2_z_ray, cs2_opalbar_ray, cs2_opac_ray, cs2_sline_ray, cs2_scont_ray, &
                  cs2_velz_ray, cs2_imask_ray, cs2_vth_ray, cs2_temp_ray
!
implicit none
!
! ... arguments
...
!-----------------------------------------------------------------------
!
if(allocated(cs2_z_ray)) deallocate(cs2_z_ray)
if(allocated(cs2_opalbar_ray)) deallocate(cs2_opalbar_ray)
if(allocated(cs2_opac_ray)) deallocate(cs2_opac_ray)
if(allocated(cs2_sline_ray)) deallocate(cs2_sline_ray)
if(allocated(cs2_scont_ray)) deallocate(cs2_scont_ray)
if(allocated(cs2_velz_ray)) deallocate(cs2_velz_ray)
if(allocated(cs2_vth_ray)) deallocate(cs2_vth_ray)
if(allocated(cs2_temp_ray)) deallocate(cs2_temp_ray)
if(allocated(cs2_imask_ray)) deallocate(cs2_imask_ray)
!
!-----------------------------------------------------------------------
!
allocate(cs2_z_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_z_ray'
allocate(cs2_opalbar_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_opalbar_ray'
allocate(cs2_opac_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_opac_ray'
allocate(cs2_sline_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_sline_ray'
allocate(cs2_scont_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_scont_ray'
allocate(cs2_velz_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_velz_ray'
allocate(cs2_vth_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_vth_ray'
allocate(cs2_temp_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_temp_ray'
allocate(cs2_imask_ray(cs2_nz_ray), stat=err)
   if(err.ne.0) stop 'allocation error allocate_fs1d: cs2_imask_ray'


end subroutine allocate_fs1dcs2
...
!-----------------------------------------------------------------------
!
deallocate(cs2_z_ray)
deallocate(cs2_opalbar_ray)
deallocate(cs2_opac_ray)
deallocate(cs2_sline_ray)
deallocate(cs2_scont_ray)
deallocate(cs2_velz_ray)
deallocate(cs2_vth_ray)
deallocate(cs2_temp_ray)
deallocate(cs2_imask_ray)
!
!
!
end subroutine deallocate_fs1dcs2
