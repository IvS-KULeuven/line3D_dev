pro benchmark03, muindx=muindx, windx=WINDX, oname=ONAME, xlim=XLIM, ylim=YLIM, clim=CLIM, logscale=LOGSCALE
;
;plots contours of searchlight along a direction
;
;------------------read all information from hdf5-file------------------
;
fname='benchmark03.h5'
;
...
;
   group_id = h5g_open(file_id, 'searchlight2d')
      dset_id=h5d_open(group_id, 'int2dsc_angdep')
         int2dsc_angdep=h5d_read(dset_id)
      h5d_close, dset_id
      dset_id=h5d_open(group_id, 'int2dfvm_angdep')
         int2dfvm_angdep=h5d_read(dset_id)
      h5d_close, dset_id
   h5g_close, group_id
;
h5f_close, file_id
...
if(not keyword_set(muindx)) then muindx=0
...
int2d_sc=int2dsc_angdep(*,*,indx_mu)
int2d_fvm=int2dfvm_angdep(*,*,indx_mu)
;
;----------------------calculate profile along a line-------------------
;
iperp, int2d_sc, x, z, mu, coord_line, xcoord_line, zcoord_line, nd, int1dsc_perp, int1d_theo
iperp, int2d_fvm, x, z, mu, coord_line2, xcoord_line2, zcoord_line2, nd2, int1dfvm_perp, int1d_theo2
;
...
if(keyword_set(oname)) then begin
   oname='ps_files/benchmark03_a.ps'
...
loadct, 0
if(keyword_set(oname)) then begin
   oname='ps_files/benchmark03_b.ps'
   print, "writing output to: ", oname
   set_plot,'ps'
   device,file=oname, $ ;xsize=19., ysize=26.7, xoffset=1., yoffset=1. , $
    decomposed=0, color=1, bits_per_pixel=8
